...
if (typeof oneCatBalanced == 'undefined') {
    var oneCatBalanced = false;
}
if (typeof catBalancedMultiplier == 'undefined') {
    var catBalancedMultiplier = 1;
}
...
<hr>
<center>
<p><font color="${titleColor}">Creator: </font><a href="https://www.shinko-to-kuma.com" style="text-shadow:-1px -1px 0 ${titleColor},1px -1px 0 ${titleColor},-1px 1px 0 ${titleColor},1px 1px 0 ${titleColor};" title="Sophie profile" target="_blank">Sophie "Shinko to Kuma"</a>
</p>
</center>
</div>`;
...
function fillInTroops() {
    ...
    if (balanced == false) {
        ...
        if (oneCatBalanced == false) {
            ...
            if (troopCounts['catapult'] < catBalancedMultiplier) {
                if(troopCounts['ram']>=catBalancedMultiplier)
                {
                    doc.forms[0]['ram'].value =catBalancedMultiplier;
                    average = (fakePopNeeded - (catBalancedMultiplier*5)) / popFakeBalance;
                    var popPerUnit = 0;
                    for (unit in troopArray) {
                        // find popcount per unit
                        switch (String(unit)) {
                ...
                {
                    alert("No rams and no cats");
                }
            }
            else {
                doc.forms[0]['catapult'].value =catBalancedMultiplier;
                average = (fakePopNeeded - (8*catBalancedMultiplier)) / popFakeBalance;
                var popPerUnit = 0;
                for (unit in troopArray) {
                    // find popcount per unit
                    switch (String(unit)) {
