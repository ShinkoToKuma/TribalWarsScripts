...
$.get(URLReq, function (data) {
    ...
    scavCatNames = JSON.parse("[" + $(data).find('script:contains("ScavengeMassScreen")')[0].innerHTML.match(/\{.*\:\{.*\:.*\}\}/g) + "]")[0];
    ...
    .done(function () {
            html = "<div><table class='sophHeader'><tr class='sophHeader'><td class='sophHeader'colspan='5'><h1><center>Mass scavenging overview</center></h1></td></tr><tr class='sophHeader'><td class='sophHeader'>Village</td><td class='sophHeader'>" + scavCatNames[1].name + "</td><td class='sophHeader'>" + scavCatNames[2].name + "</td><td class='sophHeader'>" + scavCatNames[3].name + "</td><td class='sophHeader'>" + scavCatNames[4].name + "</td></tr>";
            ...
            $.getAll(URLs,
                ...
                () => {
                    ...
                    arrayWithData += "]";
                    scavengeOverview = JSON.parse(arrayWithData);

                    //get all the data in a table
                    $.each(scavengeOverview, function (villageNr) {
                        if (villageNr % 2 == 0) {
                        ...
                        html += `<tr ${rowClass}><td class="sophHeader">${scavengeOverview[villageNr].village_name}</td>`;
                        $.each(scavengeOverview[villageNr]["options"], function (villageCategoryNr) {

                            if (scavengeOverview[villageNr]["options"][villageCategoryNr]["scavenging_squad"] != null) {
                                endTime = parseInt(scavengeOverview[villageNr]["options"][villageCategoryNr]["scavenging_squad"]["return_time"]) * 1000;
                                html += `<td ${rowClass}><span class="timer" data-endtime=${parseInt(endTime / 1000)}></span></td>`
                            }
                            else {
                                if (scavengeOverview[villageNr]["options"][villageCategoryNr]["is_locked"] != true) {
